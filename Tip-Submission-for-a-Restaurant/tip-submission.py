# Python script for hacktoberfest repo

import random

# Input from server
server_name = input(
    "Hello Kick Ass Steak House servant, please enter your name.\n")
sub_total = float(input(
    f"Thank you {server_name}, please enter the subtotal of the patron's meal without the dollar sign ($)\n"))
rounded_subtotal = float(f"{sub_total:.2f} \n")

server_passover = input(
    f"Great {server_name}.  Now hit [ENTER] and pass the device to the patron\n")

# Input from patron (if exists)
patron_name = input(
    "We hope you enjoyed your meal at Kick Asss Steak House, can I please get your name?\n")
...
see_thoughts = input(
    "BONUS: Hit [ENTER] to see what our waiters & waitresses really thought of their patrons\n")
